// we cand add the function testHeadPosition to make the snake dead when it hits the wall 
...
const v =1;// it should be{1,2,3,5,10,}
let food = {x:300,y:20}
...
// if we want to restart the game when it hits the wall
// we remove from the function moveRight left up and down the if condition
function testHeadPosition(){
    const { width, height } = getCanvasSize();
    if (body[0].x >= width || body[0].x < 0 || body[0].y>=height || body[0].y < 0){
...
function testPosition(){
    if (body[0].x%20==0 && body[0].y%20==0){
        return true;
    }
    else {return false;}
}
function testNewDirection(){
    let i =0 ;
    while (body[i].newDirection == body[i+1].newDirection && i<body.length-3){
        i++;
    }
...
function changePosition() {
    ...
    if ( body[0].newDirection != newDirection){
        if (testNewDirection() && testPosition() ){
